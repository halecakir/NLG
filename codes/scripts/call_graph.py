...
"""
List 3rd-Party libs
def list_third_party_libs(node, libs):
    for child in node.childs:
        if not child.path.startswith(thirdparty_analyzer.custom_path):
            module = child.class_name.split()[-1]
            libs.add(module)
            list_third_party_libs(child, libs)

libs = set()
for node in app_nodes[key]:
    list_third_party_libs(node, libs)
libs
"""